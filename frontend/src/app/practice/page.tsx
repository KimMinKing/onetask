...
export default function PracticePage() {
  ...
  useEffect(() => { Promise.all([api.words.list(4), api.englishWords.list(), api.japaneseWords.list("N5")]).then(([zh,en,ja]) => setCards([...zh.slice(0,5).map(w=>({id:`zh-${w.id}`,text:w.chinese,sub:`${w.pinyin} · ${w.meaning}`,lang:"zh-CN"})),...en.slice(0,5).map(w=>({id:`en-${w.id}`,text:w.word,sub:w.meaning,lang:"en-US"})),...ja.slice(0,5).map(w=>({id:`ja-${w.id}`,text:w.expression,sub:`${w.reading} · ${w.meaning}`,lang:"ja-JP"}))])); }, []);
  ...
  const finish = async () => { if (!eventId.current) eventId.current = crypto.randomUUID(); await api.learning.activity({ client_event_id:eventId.current, subject:"languages", activity_type:mode, title:"듣기·말하기 연속 연습", duration_minutes:10, total_count:cards.length }); router.push("/"); };
