...
export default function NetworkPage(){
  ...
  useEffect(()=>{setProgress(readProgress());api.learning.progress("network").then(rows=>setProgress(prev=>({...prev,...Object.fromEntries(rows.filter(row=>row.completed).map(row=>[Number(row.unit_id),true]))}))).catch(()=>{});const step=Number(new URLSearchParams(window.location.search).get("step"));if(step>=1&&step<=100)setSelected(step);},[]);
  ...
  const finish=async(id:number)=>{const next={...progress,[id]:true};setProgress(next);localStorage.setItem(STORAGE_KEY,JSON.stringify(next));await api.learning.complete("network",String(id),{completed:true,score:100,duration_minutes:15,title:NETWORK_CURRICULUM[id-1].title}).catch(()=>{});setSelected(id<100?id+1:0);};
  if(current)return <Lesson lesson={current} done={!!progress[current.id]} move={setSelected} complete={()=>finish(current.id)} onWrong={(choice)=>api.learning.addMistake({subject:"network",item_key:`lesson-${current.id}`,question:current.quiz.question,correct_answer:current.quiz.options[current.quiz.answer],user_answer:current.quiz.options[choice],explanation:current.quiz.explanation}).catch(()=>{})}/>;
