...
export function SentenceCompletionQuiz({ wordLang = "zh", onBack, onDone }: {
  wordLang?: string;
  onBack: () => void;
  onDone: () => void;
}) {
  ...
  const loadQuizzes = async () => {
    ...
    for (let i = 0; i < 5; i++) {
      try {
        const q = await api.quizzes.sentenceCompletion(wordLang);
        loaded.push(q);
      } catch {
        break;
      }
    ...
    return (
      <div className="flex flex-col min-h-dvh bg-dark-400 items-center justify-center px-6 gap-6">
        ...
        <div className="text-center">
          ...
          <p className="text-stone-500 text-sm mt-1">문장 완성 퀴즈 결과</p>
        </div>
  ...
  return (
    ...
    >
      <div className="w-full space-y-3">
        <p className="text-center text-lg text-stone-300">빈칸에 들어갈 단어를 선택하세요</p>
