    ...
    try:
        ...
        for settings in rows:
            ...
            if not tasks:
                continue
            try:
                send_message(settings.telegram_bot_token, settings.telegram_chat_id, format_due_tasks_message(tasks))
...
def job_telegram_daily_quiz():
    ...
    try:
        ...
        for settings in rows:
            ...
            try:
                send_message(settings.telegram_bot_token, settings.telegram_chat_id, f"<b>☀️ 오늘의 {plan['total_minutes']}분 학습</b>\n\n{lines}\n\n<a href=\"{base_url}/\">지금 이어서 학습하기 →</a>")
                if quizzes:
                    send_message(settings.telegram_bot_token, settings.telegram_chat_id, f"<b>🧠 어제 배운 단어 복습</b>\n\n{len(quizzes)}문제만 가볍게 풀어보세요.")
                    for quiz in quizzes:
                        send_quiz_poll(settings.telegram_bot_token, settings.telegram_chat_id, quiz.question, quiz.options, quiz.correct_option_id, quiz.explanation)
            except TelegramError as exc:
